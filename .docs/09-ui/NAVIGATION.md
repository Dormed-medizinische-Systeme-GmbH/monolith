...
Gilt zunächst für `erp.dormed.de` (aktueller Bau-Fokus, ADR-020). Die Verarbeitung —
Permission-Prüfung, Cockpit-Definitionen — liegt in `core`; die App
...
| **CRM** | **Anfragen** — mit **Zähler** unverknüpfter Anfragen (D-132) | backoffice · management |
...
| **Billing** | Rechnungen · Zahlungen | backoffice · management (war `accounting`, D-125) |
| **Billing** | **Mahnwesen** | backoffice · **geschaeftsfuehrung** — **nicht** management (D-138) |
| *(fix, unten)* | ⚙ Einstellungen | **alle** |

> **Mahnwesen ist der einzige Eintrag, den Management nicht sieht** (D-138). Weil die
> Nav aus dem Permission-Katalog abgeleitet wird (D-123), verschwindet der Eintrag für
> Management automatisch — es braucht keine Sonderregel im Menü.
...
| # | Punkt | Wohin |
| --- | --- | --- |
| 1 | ~~Genaue Permissions je Abteilung~~ | ✅ gelöst — Matrix in [`../03-security/IDENTITY_RBAC.md`](../03-security/IDENTITY_RBAC.md) (D-136–D-140) |
...
| 3 | ~~Abgrenzung Geschäftsführung ↔ Management~~ | ✅ gelöst — nur Mahnwesen ist GF-exklusiv (D-138) |
...
| 7 | Zähler-Mechanik allgemein: „Anfragen" (D-132) ist der erste Nav-Eintrag mit Badge — ob weitere Einträge Zähler bekommen, ist offen | bei Umsetzung |
| 8 | SSO-Folge: Entra-`roles`-Claim-Sync muss auf **ein** Rollen-Mapping umgestellt werden; mehrere App-Rollen je Nutzer werden Fehlerfall | vor SSO-Slice (D-029/D-124) |
