...
> `core` + `erp.dormed.de` (ADR-020); die übrigen drei Container existieren vorerst
...
```text
website container   (dormed.de, Blade — Platzhalter)
shop container       (shop.dormed.de, Inertia+Svelte — Platzhalter, Migration später)
crm container        (erp.dormed.de, Inertia+Svelte — aktueller Bau-Fokus)
portal container     (my.dormed.de — Platzhalter)
...
Einmal-Schritt gegen `database/migrations` (ADR-015):
...
```text
127.0.0.1 dormed.test erp.dormed.test portal.dormed.test shop.dormed.test
```

Live-Domains (Prod, ADR-021): `dormed.de` (Website), `my.dormed.de` (Portal),
`shop.dormed.de` (Shop), `erp.dormed.de` (ERP — Mitarbeiter-Anwendung, ADR-031).
