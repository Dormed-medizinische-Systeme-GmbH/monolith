...
Modul: `app/Modules/Billing/` (Namespace `App\Modules\Billing\`,
`depends_on: [Core, Service, Sales]`, ADR-033).
...
Ausbau bei Bedarf (ADR-010). Mahn-**Dokumente** (PDF) → [`DOCUMENTS.md`](DOCUMENTS.md),
Blade-Template im Belegarten-Katalog (D-144).

> **Keine archivierten Rechnungs-PDFs (D-142).** Auch die gestellte Rechnung wird bei
> Bedarf neu gerendert; es wird kein PDF abgelegt und keine Template-Version gepinnt.
> Die GoBD verlangt Unveränderbarkeit der **gebuchten Daten** — die sind nach D-093
> eingefroren — nicht ein eingefrorenes Layout. Begründung und Nachrüstweg: `DOCUMENTS.md`.
