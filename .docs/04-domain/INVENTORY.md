...
Autoritative deklarative Spec. Entscheidungen **D-099 – D-121**, **D-128 – D-131**, **D-134 – D-135**
([`../07-decisions/grill-log.md`](../07-decisions/grill-log.md)).
Prinzipien: [`../05-modules/INVENTORY.md`](../05-modules/INVENTORY.md).

Modul: `app/Modules/Inventory/` (Namespace `App\Modules\Inventory\`,
`depends_on: [Core]`, ADR-033).
...
**`article_group_fields`** — die Definition:

| Feld | Typ | Notiz |
| --- | --- | --- |
| `article_group_id` | FK | |
| `key` / `label` | string | technischer Schlüssel + Anzeigename |
| `type` | enum | siehe Typtabelle. **UNIQUE `(id, type)`** — Anker für den FK der Wertetabellen (D-134) |
...
**`article_group_field_options`** — die Werteliste bei `type = select` (D-134,
ersetzt ein früheres `options`-JSON):

| Feld | Typ | Notiz |
| --- | --- | --- |
| `field_id` | FK → `article_group_fields` | cascade |
| `value` | string | technischer Wert |
| `label` | string | Anzeigetext |
| `position` | smallint | Reihenfolge |
| `is_active` | boolean | |
...
### Wo die Werte liegen (D-134)

**Normalisierte Wertetabellen, kein JSONB.** Getrennt nach `scope` (D-119), damit
jede einen **echten** Fremdschlüssel hat statt einer polymorphen Beziehung:
...
Je Zeile **typisierte Wertespalten** statt einer `value`-Spalte:

`value_string` · `value_text` · `value_integer` · `value_decimal` · `value_date`
· `value_boolean` · `value_option_id` (FK → `…_field_options`)

mit **CHECK: genau eine** davon gesetzt.

> Eine einzelne VARCHAR-`value`-Spalte wäre der klassische EAV-Fehler — sie gibt
> genau die Typisierung auf, die D-094 und ADR-007 einfordern. Sechs leere Spalten
> und eine gefüllte sind dagegen eine **typisierte Union**: keine Redundanz, keine
> transitive Abhängigkeit, also keine Normalisierungsverletzung. Dieselbe
> Kategorie-Unterscheidung wie in D-093 (Snapshots sind keine Denormalisierung).

**`select`-Werte referenzieren die Option, sie kopieren sie nicht.** Wird eine
Option umbenannt, ändert sich die Bezeichnung überall mit, statt in kopierten
Strings zurückzubleiben.

**Wie Postgres den richtigen Typ erzwingt:** „genau eine Wertespalte gesetzt" ist
in-row prüfbar, „und zwar die passende zum `type`" nicht — der `type` steht in der
Definitionstabelle. Die Wertetabelle führt `type` deshalb mit und bindet es über
einen **zusammengesetzten Fremdschlüssel** `(field_id, type)` →
`article_group_fields (id, type)`. Die mitgeführte Kopie kann konstruktionsbedingt
nicht divergieren; die Alternative (Trigger oder reine Anwendungsvalidierung) wäre
schwächer als das, was ADR-007 verlangt.
...
Der Feldkatalog der `OfferingGroup` ist **strukturgleich** zum Artikel-Feldkatalog
und ebenso vollständig normalisiert (D-134): `offering_group_fields` (Definition,
ohne `scope`) → `offering_group_field_options` (Auswahllisten) →
`offering_field_values` (typisierte Werte).
