...
Modul: `app/Modules/Service/` (Namespace `App\Modules\Service\`, `depends_on: [Core, Inventory]`, ADR-033 — Inventory neu durch D-109/D-121).
...
| `signature_image` | `bytea` | ✓ | **in der Datenbank**, nicht im Object Storage (ADR-028) — klein (5–30 KB), atomar am Bericht, vom DB-Backup erfasst |
| `signer_name` | string | ✓ | |
| `signed_at` | datetime | ✓ | |
...
Fotos/Nachweise → **`Attachment`**, polymorph am Report
([`DOCUMENTS.md`](DOCUMENTS.md), D-143). **Ausdrücklich nicht `documents`** — ein
Dokument ist nach D-142/D-143 ein erzeugter Beleg und existiert gar nicht als
Datensatz; Fotos sind etwas anderes.

**Ablage auf einem gemounteten Volume** unter `erp.dormed.de` →
`storage/app/private/...`, ausgeliefert über eine Policy-geprüfte Route (ADR-029) —
**nicht** im öffentlichen Object-Storage-Bucket (ADR-028) und wegen der Größe auch
nicht als `bytea`. Die Aufräumregel bleibt vertagt (D-145).
