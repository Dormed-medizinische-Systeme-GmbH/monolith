...
> **Nachtrag 2026-09-13 — Pfade und Benennung in älteren Einträgen.** Dieses Log ist
> eine **Audit-Spur** und wird rückwirkend **nicht** umgeschrieben. Zwei Dinge haben
> sich seit den frühen Einträgen geändert (ADR-030/ADR-031):
>
> - `packages/core` heißt jetzt **`core/`**, `apps/<app>` heißt **`<domain>/`** im
>   Wurzelverzeichnis (`dormed.de`, `erp.dormed.de`, `my.dormed.de`, `shop.dormed.de`).
> - Die Mitarbeiter-Anwendung heißt **ERP**, nicht mehr CRM. **CRM** bezeichnet ab
>   ADR-031 nur noch die Domäne (Firmen/Personen/Kontakte); Modulname `Crm`,
>   Namespace und Permission-Präfix `crm.*` bleiben unverändert korrekt.
>
> Ältere Einträge nennen weiterhin die alten Pfade. Maßgeblich ist
...
**Konkrete Form:** Feldtypen und Feldoptionen → **D-111**, Feldebene (`scope`) →
**D-119**, Speicherung (EAV vs. JSONB) → **D-134**: normalisierte Tabellen mit
typisierten Wertespalten, **kein** JSONB.
...
**Status:** **in zwei Stufen geschnitten (2026-09-13)** — Stufe 1 entschieden
(D-132), Automatik vertagt · **Datum:** 2026-09-13 · Bereich Communication/CRM

> **Scope-Korrektur durch den Nutzer (2026-09-13).** Die automatische Verknüpfung
> soll **erst später** kommen, „wenn das System schon läuft": „Sollte ja easy
> extendable sein, die Logik eines abgesendeten Kontaktformulars abzulegen und
> dann später das Verlinken dranzuhängen. Fürs erste gibt es dann nur eine
> Anfrage, die dann verknüpft werden muss."
>
> Damit zerfällt dieser Eintrag in **Stufe 1** (Anfrage speichern + manuell
> verknüpfen → **D-132**) und **Stufe 2** (Matching, Auto-Anlage → bleibt hier
> offen). Die drei unten benannten Spannungen gelten weiterhin — **zwei davon
> aber erst für Stufe 2**, siehe D-132.
...
## Bereich: Communication — Kontaktformular (Runde 2026-09-13)

Erste Runde im Bereich Communication. Ergebnis:
[`../04-domain/COMMUNICATION.md`](../04-domain/COMMUNICATION.md).

**Ist-Referenz, neu verfügbar:** der Nutzer hat das produktive Website-Projekt
unter `dormed.de/` im Repo-Root abgelegt (**nur lesen, nicht anfassen** — es ist
nicht Teil dieses Repos und wird später eigenständig hereinmigriert). Damit
existiert für Communication erstmals eine **echte, lebende Quelle** statt nur
Nutzerangaben — anders als bei Inventory (D-108). Relevante Dateien:
`app/Http/Requests/ContactFormRequest.php`, `app/Http/Controllers/ContactFormController.php`,
`app/Jobs/CreateInquiryInCas.php`, `resources/views/kontakt.blade.php`.

### D-132 — `ContactRequest`: Anfrage speichern und manuell verknüpfen (Stufe 1 von D-122)

**Status:** entschieden · **Datum:** 2026-09-13 · **schneidet D-122 auf**

**Scope-Korrektur des Nutzers:** die automatische Verknüpfung kommt **erst
später**, „wenn das System schon läuft" — „sollte ja easy extendable sein, die
Logik eines abgesendeten Kontaktformulars abzulegen und dann später das Verlinken
dranzuhängen. Fürs erste gibt es dann nur eine Anfrage, die dann verknüpft werden
muss."

**Das löst nebenbei den D-002-Konflikt.** Die Anfrage **ist** die Zwischenstufe,
die ich als fehlend markiert hatte: ein Formulareingang landet als eigenständiger
Datensatz, **ohne** dass eine `Person` oder `Company` entsteht. D-002 („Person
existiert nie ohne Company") bleibt damit unangetastet — es braucht keine Ausnahme.
Das reale Formular bestätigt die Notwendigkeit: **`praxis` ist optional**, eine
Anfrage ohne Firmenangabe ist der Normalfall.

**Kein API-Job.** Das Altsystem schickt die Anfrage per `CreateInquiryInCas` an ein
CAS-`Inquiries`-Objekt. Diese Anbindung ist im Zielsystem **hinfällig** (Nutzer) —
der Monolith ist selbst das CRM, die Anfrage wird direkt in die eigene Datenbank
geschrieben. Übernommen wird nur die **Feldlogik**, nicht der Übertragungsweg.
Bemerkenswert bleibt: ein eigenständiges Anfrage-Objekt existiert im Altsystem
bereits — die Entität ist kein neues Konstrukt.

**Felder** — verbatim aus `ContactFormRequest::rules()` plus Legacy-Zielfeld:

| Formularfeld | Regel | CAS-Feld | Zielmodell |
| --- | --- | --- | --- |
| `name` | required, max 128 | `Name` | `name` |
| `email` | required, email, max 64 | `MAIL` | `email` |
| `telefon` | nullable, max 32 | `PHONE` | `phone` |
| `plz` | **required, digits:5** | `ZIP` | `postal_code` |
| `nachricht` | nullable, max 5000 | `MESSAGE` | `message` |
| `praxis` | nullable, max 64 | `COMPANY` | `company_name` |
| `fachgebiet` | nullable, Auswahlliste | `SPECIALTY` | `medical_specialty_id` (D-133) |
| `rueckruf` | nullable, `in:ja` | `CALLBACK_REQUEST` | `callback_requested` |
| `rueckruf_datum` | nullable, date | `CALLBACK_DATE` | `callback_date` |
| `datenschutz` | **required, `in:ja`** | — (nur Gate) | → `Consent` (D-013) |

**Drei Ableitungen aus den realen Feldern:**

1. **`plz` ist Pflicht** — damit kann die Zuständigkeit schon in Stufe 1 über
   `sales_territories` (D-084) **vorbelegt** werden, ganz ohne Matching-Logik. Die
   Anfrage landet beim richtigen Vertriebler, auch wenn sie noch keinem Kunden
   zugeordnet ist. Das ist **keine** vorgezogene Automatik aus Stufe 2: es wird
   eine Zuständigkeit gesetzt, **kein Datensatz verknüpft**.
2. **`datenschutz`** ist die DSGVO-Einwilligung. Sie wird an der Anfrage
   festgehalten (Zeitpunkt + Text-/Versionsstand) und beim späteren Anlegen eines
   Kundenstamms in einen `Consent` (D-013, historisiert, kanalweise) überführt —
   sonst entstünde ein Datensatz ohne Rechtsgrundlage für die Kontaktaufnahme.
3. **Jedes Feld bekommt eine eigene Spalte** — **Korrektur des Nutzers,
   2026-09-13.** Der Agent hatte zusätzlich ein `payload`-JSON mit den Rohdaten
   der Übermittlung vorgeschlagen, als Auffangnetz für spätere Formularänderungen.
   Der Nutzer hat das **verworfen**: „Das mit dem JSON in der Datenbank finde ich
   nicht gut. Das ist nicht, was ich mit extendable meine — für jedes Feld eine
   DB-Spalte."

   **Der Nutzer hat recht, und zwar aus dem Projekt heraus:** ein schemaloser
   JSON-Blob widerspricht **D-093** (höchstmögliche Normalisierung, keine
   Ausweichstrukturen) und **D-094** (typisierte Spalte + CHECK-Constraint, jede
   Änderung eine eigene Migration und eine eigene `D-NNN` — „bewusster
   Reibungspunkt"). Der Vorschlag hätte für Kontaktanfragen eine Ausnahme
   geschaffen, für die es keine Begründung gibt: anders als beim
   Artikel-Feldkatalog (D-100) definiert hier **niemand im UI** Felder — das
   Formular ändert ohnehin nur ein Entwickler.

   **Damit gilt:** `ContactRequest` hat für **jedes** Formularfeld eine eigene,
   typisierte Spalte. Kommt ein Feld hinzu, ist das eine Migration + eine eigene
   `D-NNN`. **Stufe 2 matcht auf diesen Spalten** (`email`, `phone`,
   `postal_code`, `name`, `company_name`) — was als Matching-Grundlage ohnehin
   trägt, dafür brauchte es nie Rohdaten.

**Status:** `neu` → `verknuepft` → `verworfen` (Endzustand, für Spam/Irrläufer).
Nach dem Verknüpfen **bleibt die Anfrage stehen** (Nutzerentscheidung) — sie hängt
an der Company bzw. am Kontakt und bleibt Ursprungsbeleg. Was daraus wird
(Verkaufschance, Servicefall, nichts), entscheidet der Bearbeiter **separat**;
es wird nichts automatisch erzeugt — konsistent mit **D-053** („gewonnen erzeugt
nichts automatisch").

**Zuständig:** `backoffice` und `management` (Nutzerauswahl).

**„Keine verwaisten Anfragen" — wie es ohne Automatik sichergestellt wird:**
**beides** (Nutzerauswahl) — ein **Nav-Eintrag „Anfragen" mit Zähler** der
unverknüpften plus eine **Cockpit-Kachel** (D-123/D-126). Der Zähler verschwindet
erst, wenn abgearbeitet ist. Die strukturelle Garantie aus D-122 wird damit in
Stufe 1 nicht durch Automatik, sondern durch **Unübersehbarkeit** erreicht.

**Weiterhin offen (Stufe 2, bleibt an D-122):** Matching-Kriterien,
Mehrfachtrefferverhalten, automatische Anlage eines Kundenstamms und die dafür
nötige Spam-Abwehr. Letztere ist in Stufe 1 **entschärft**, aber nicht erledigt:
eine Spam-Anfrage erzeugt keinen Kundenstamm mehr, nur eine Zeile, die auf
`verworfen` gesetzt wird.

### D-133 — Fachgebiete: **eine** Liste; `MedicalSpecialty`-Seed wird erweitert

**Status:** entschieden · **Datum:** 2026-09-13 · **erweitert D-019**

**Der Konflikt:** die Auswahlliste im Website-Formular
(`resources/views/kontakt.blade.php`) und der `MedicalSpecialty`-Seed aus D-019
sind zwei unterschiedliche Listen — 8 gegen 21 Werte, abweichende Benennung, und
„Orthopädie / Sportmedizin" ist im CRM in **zwei** Einträge getrennt.

**Entscheidung:** **eine einzige Liste.** `MedicalSpecialty` bleibt die Wahrheit,
das Formular zieht seine Auswahl **aus der Tabelle** statt aus einem
hartcodierten Array. Der D-019-Seed wird um die fehlenden Website-Werte erweitert:

- **Allgemeinmedizin / Hausarzt**
- **Innere Medizin**
- **Gynäkologie**
- **Urologie**
- **Sonstiges** — *Agent-Ergänzung:* der D-019-Seed hat keinen Auffangwert, das
  Formular braucht aber einen. Ohne ihn müsste ein Besucher außerhalb der Liste
  das Feld leer lassen.

`MedicalSpecialty` ist laut CORE.md ohnehin eine **vom Nutzer pflegbare** Liste —
die Erweiterung ist deshalb kein Bruch, sondern der vorgesehene Weg.

**Offen (Kosmetik, nicht blockierend):** die Benennung wird dadurch gemischt — der
Altsystem-Seed nennt Personen im Plural („Orthopäden", „Kardiologen"), die
Website-Werte nennen Fachgebiete („Gynäkologie", „Innere Medizin"). Eine
Vereinheitlichung wäre sinnvoll, ist aber eine eigene Entscheidung und berührt
Bestandsdaten.

### D-134 — Feldkatalog vollständig normalisiert: Optionen und Werte als eigene Tabellen

**Status:** entschieden · **Datum:** 2026-09-13 · **schließt den offenen Punkt aus D-100**
· **konkretisiert D-111/D-119**

**Auslöser:** Bei der Streichung des `payload`-JSON (D-132) fiel auf, dass der
Feldkatalog selbst noch ein JSON-Feld trägt (`options`). Der Nutzer hat die Regel
daraufhin allgemein gezogen: „Dann sollten wir das vermutlich machen, also auch
normalisieren. Weil ich möchte das erste Mal nicht auf meinen Bauch hören, sondern
einfach maximal normalisiert."

Damit ist zugleich der seit D-100 offene Punkt **„Speicherung EAV vs. JSONB"**
beantwortet: **normalisierte Tabellen, kein JSONB.**

#### 1. Auswahllisten → eigene Tabelle

`article_group_fields.options` (json) **entfällt**. Stattdessen
`article_group_field_options`: `field_id`, `value`, `label`, `position`, `is_active`.

#### 2. Feldwerte → eigene Tabellen, typisiert

Die Werte liegen in Wertetabellen, **getrennt nach `scope`** (D-119) — damit jede
einen **echten** Fremdschlüssel hat statt einer polymorphen Beziehung (ADR-007,
Postgres als Integritätsgrenze):

| Tabelle | `scope` | Bezug |
| --- | --- | --- |
| `article_field_values` | `article` | `article_id` → `articles` |
| `device_field_values` | `item` | `device_id` → `devices` |
| `offering_field_values` | — (nur eine Stufe, D-117) | `offering_id` → `offerings` |

**Typisierte Wertespalten statt einer `value`-Spalte.** Eine einzelne
VARCHAR-`value`-Spalte wäre der klassische EAV-Fehler: sie gibt genau die Typisierung
auf, die D-094 und ADR-007 einfordern. Stattdessen je Zeile:

`value_string` · `value_text` · `value_integer` · `value_decimal` · `value_date`
· `value_boolean` · `value_option_id`

mit **CHECK: genau eine** dieser Spalten ist gesetzt.

> Das ist **keine** Normalisierungsverletzung: sechs leere Spalten und eine
> gefüllte sind eine typisierte Union, keine Redundanz und keine transitive
> Abhängigkeit. Dieselbe Kategorie-Unterscheidung wie in **D-093** (Snapshots sind
> keine Denormalisierung).

#### 3. `select`-Werte referenzieren die Option, sie kopieren sie nicht

Bei `type = select` wird **nicht der Text gespeichert**, sondern
`value_option_id` → `article_group_field_options`. Das ist der eigentliche Gewinn
dieser Runde: wird eine Option umbenannt, ändert sich die Bezeichnung überall
mit, statt in kopierten Strings zurückzubleiben.

#### 4. Wie Postgres den *richtigen* Typ erzwingt

„Genau eine Wertespalte gesetzt" ist in-row prüfbar. „Und zwar die, die zum `type`
der Felddefinition passt" nicht — der `type` steht in einer anderen Tabelle.
Lösung: die Wertetabelle führt `type` mit und bindet es über einen
**zusammengesetzten Fremdschlüssel** `(field_id, type)` → `article_group_fields
(id, type)` (dort UNIQUE). Der CHECK kann `type` dann in-row auswerten.

> Das mitgeführte `type` ist eine **durch Fremdschlüssel garantierte** Kopie — sie
> kann konstruktionsbedingt nicht divergieren. Die Alternative wäre ein Trigger
> oder reine Anwendungsvalidierung; beides wäre schwächer als das, was ADR-007
> verlangt.

#### Preis, bewusst getragen

Der Feldkatalog kostet jetzt **sieben** Tabellen statt zwei (Definitionen +
Optionen + Werte, je für Artikel und Leistungen). Das Auslesen aller Felder eines
Artikels ist ein Join statt eines Spaltenzugriffs. Der Nutzer hat das ausdrücklich
gegen den eigenen Bauch entschieden — die Konsistenz mit D-093/D-094 wiegt schwerer
als der Zugriffskomfort.

**Nicht betroffen** bleiben die in D-093 begründeten Zustands-Snapshots
(`Invoice.recipient_*`, `Invoice`-Summen, `ServiceContract.maintenance_price`,
`MaintenanceDevice.maintenance_fee_snapshot`) sowie die Preis-Snapshots aus D-104.
Das sind rechtlich/fachlich geforderte Festschreibungen zu einem Zeitpunkt, keine
redundanten Ableitungen — „maximal normalisiert" ändert daran nichts.

**Nachtrag:** `article_group_field_options` und `offering_group_field_options` sind
strukturell identisch, ebenso die beiden Definitionstabellen. Eine Zusammenlegung
zu **einem** Katalog-Feldsystem wurde erwogen und **verworfen** — siehe **D-135**.
Die Dopplung bleibt bewusst bestehen.

### D-135 — Keine Zusammenlegung von Artikel- und Leistungskatalog (bestätigt D-117)

**Status:** entschieden · **Datum:** 2026-09-13 · **schließt den offenen Punkt aus D-134**

D-134 hatte als Randbemerkung offengelassen, ob die strukturgleichen
Katalog-Feldtabellen für Artikel und Leistungen zu **einem** gemeinsamen
Feldsystem zusammengelegt werden sollten — bis hin zu einem vereinten Katalog, in
dem eine Leistung nur ein Artikel ohne Physis wäre.

**Der Nutzer hat das nach Abwägung verworfen:** „Ich folge nicht deiner Empfehlung
einer Zusammenlegung in einen gemeinsamen Katalog."

**Damit gilt D-117 unverändert weiter:**

- `ArticleGroup → Article → Exemplar` (3 Stufen) und `OfferingGroup → Offering`
  (2 Stufen) bleiben **zwei getrennte, parallele Kataloge**.
- Die Feldkatalog-Tabellen bleiben je Katalog eigenständig:
  `article_group_fields` / `offering_group_fields`, dazu je eigene
  `*_field_options` und `*_field_values` (D-134).
- `line_items` und `OpportunityItem` behalten **beide** nullable Fremdschlüssel,
  `article_id` **und** `offering_id` (D-109/D-117).
- Das Positionsmodal behält seine Tabs „Leistungen / Artikel / Sonstiges" (D-116),
  die damit weiterhin auf **verschiedene** Quellen zeigen, nicht auf gefilterte
  Sichten derselben Tabelle.

Die strukturelle Ähnlichkeit der beiden Feldkataloge ist eine **bewusst getragene
Dopplung** — sie hält Artikel und Leistungen fachlich sauber getrennt, so wie
D-117 es ursprünglich begründet hat („sollten separat zu Artikelgruppen, Artikel
und Items existieren").

**Dieser Punkt ist entschieden, nicht vertagt** — er wird nicht erneut
aufgeworfen.

---

## Bereich: Permissions je Abteilung (Runde 2026-09-13)

Schließt `.docs/09-ui/NAVIGATION.md` #1, den Punkt, der die Nav-Umsetzung blockierte.
Ergebnis: Matrix in [`../03-security/IDENTITY_RBAC.md`](../03-security/IDENTITY_RBAC.md).

Der Katalog umfasst inzwischen **neun Module** mit rund 40 Ressourcen. Statt jede
Zelle einzeln zu grillen, wurde die **Konvention** festgelegt (D-136) und nur die
strittigen Zellen entschieden (D-137–D-140).

### D-136 — Aktionsvokabular: CRUD plus benannte Workflow-Abilities

**Status:** entschieden (Agent-Entscheidung, aus D-030 abgeleitet) · **Datum:** 2026-09-13

Schema bleibt `<modul>.<ressource>.<aktion>` (D-030). Aktionen:

| Aktion | Bedeutung |
| --- | --- |
| `view` · `create` · `update` | Basis |
| `delete` | **Soft Delete** — in den Papierkorb legen (D-018), **nicht** endgültig |
| `restore` | aus dem Papierkorb zurückholen |
| **benannte Workflow-Abilities** | jeder Zustandsübergang mit fachlicher Wirkung |

**Warum Workflow-Abilities eigene Namen brauchen:** Ohne sie gewährt ein pauschales
`update` implizit auch „Status auf `abgerechnet` setzen" oder „Rechnung stellen". Das
würde die dritte Ebene aus `AUTHORIZATION.md` („What may you do **now**?")
aushebeln — dort ist ausdrücklich festgehalten, dass jemand eine Wartung bearbeiten
darf, ohne jeden Zustand setzen zu dürfen.

Beispiele: `service.maintenances.complete` · `billing.invoices.issue` ·
`billing.invoices.cancel` · `sales.opportunities.close` ·
`inventory.counts.post` (Zählauftrag buchen) · `inventory.stock.override_negative`
(Minusbestand übersteuern, D-112).

Modul-Wildcards (`sales.*`) nur, wo eine Rolle den ganzen Bereich verantwortet.

### D-137 — Techniker-Inventory: vier Abilities, kein `inventory.*`

**Status:** entschieden · **Datum:** 2026-09-13 · **konkretisiert D-127**

Die Rolle `service` bekommt **genau diese** Inventory-Rechte:

| Ability | Wofür |
| --- | --- |
| `inventory.stock.view.own` | nur der Bestand **des eigenen** Technikerlagers (D-101/D-109) |
| `inventory.stock.consume` | Entnahme über das Positionsmodal (D-109) |
| `inventory.counts.submit.own` | **eigenen** Zählauftrag einreichen (D-113) |
| `inventory.pickup_notes.create` | **Abholbeleg beim Kunden** ausstellen (D-130) |
| `inventory.articles.view` | Artikelstamm **lesend** — Bezeichnung und Preis landen ohnehin auf der Position |

**Der Abholbeleg war ein Fund beim Durchsprechen der Matrix.** Der zunächst gewählte
Zuschnitt enthielt ihn nicht — D-130 verlangt aber ausdrücklich, dass der Beleg „vom
Techniker beim Kunden erstellt, vor Ort unterschrieben und mitgenommen" wird. Ohne
das Recht hätte der Techniker ein Defektgerät nicht mitnehmen können. Nutzer hat
korrigiert: der Techniker bekommt es.

**Ausdrücklich nicht:** `inventory.articles.update`, `warehouses.*`,
`goods_receipts.*`, `purchase_orders.*`, `transfers.*`, fremde Lagerbestände.

### D-138 — Geschäftsführung vs. Management: nur Mahnwesen ist GF-exklusiv

**Status:** entschieden · **Datum:** 2026-09-13 · **schließt den offenen Punkt aus D-125**

`geschaeftsfuehrung` hat `['*']`. Für `management` gilt **eine** Einschränkung:

- **Mahnwesen und Zahlungsverhalten** (`billing.dunning.*`, `billing.payments.view`,
  offene Posten und Verzug je Kunde, D-070–D-072) → **nur Geschäftsführung**.

**Was zunächst ebenfalls als sensibel markiert war und wieder freigegeben wurde:**

- **Einkaufspreise** — zurückgenommen, Nutzer: „Management sieht den EK doch."
  Begründung: Management setzt die Verkaufspreise (D-140). Wer Preise setzt, ohne
  den Einstandspreis zu kennen, kann unbemerkt unter Kosten anbieten.
- **Deckungsbeitrag** (`marginal_return`, D-052) — *Agent-Entscheidung:* ebenfalls
  frei für Management. Er ist nach D-052 eine **abgeleitete Größe** aus
  `unit_price` und `unit_cost`. Wer beide sieht, kennt die Marge — sie zusätzlich zu
  verbergen wäre Schein-Sicherheit, die nur die Oberfläche unbrauchbar macht.
- **Benutzerverwaltung** — nicht als sensibel markiert, Management darf sie.

> **Beobachtung, keine Entscheidung:** Damit unterscheiden sich `geschaeftsfuehrung`
> und `management` nur noch in einem Bereich. Ob das zwei Rollen rechtfertigt, ist
> eine Frage für später — die fünf Rollen sind gesetzt (D-125).

### D-139 — Löschmodell: Soft Delete für Management/GF, endgültiges Löschen macht ein Job (revidiert D-023/D-125)

**Status:** entschieden · **Datum:** 2026-09-13 · **revidiert D-023 und D-125**

Nutzer: „Endgültig löschen wird durch einen Job erledigt. Das Wiederherstellen dürfen
wie das Löschen nur Management, GF."

**Drei klar getrennte Ebenen:**

| Ebene | Wer |
| --- | --- |
| **anlegen / ändern** (`create`, `update`) | `backoffice` · `sales` · `service` · `management` · `geschaeftsfuehrung` |
| **löschen / wiederherstellen** (`delete`, `restore` — Papierkorb, D-018) | **nur** `management` · `geschaeftsfuehrung` |
| **endgültig entfernen** | **niemand manuell** — ein **Scheduler-Job** nach Ablauf der Aufbewahrungsfrist |

**Was sich gegenüber D-023 tatsächlich ändert** — und was **nicht**:

- **Unverändert:** Der Job war schon da. D-023 hatte bereits „Retention **30 Tage**
  fix, danach `model:prune`", und `CORE.md` nennt die 30 Tage ebenfalls. Die
  Aufbewahrungsfrist ist damit **nicht** offen, sondern seit D-023 entschieden.
- **Geändert — Backoffice verliert das Löschrecht.** D-023 gab `delete`/`restore` an
  „Management/Backoffice", D-125 schrieb `backoffice` den Papierkorb zu. Beides gilt
  nicht mehr: **nur** `management` und `geschaeftsfuehrung`.
- **Geändert — `forceDelete` entfällt als menschliche Ability.** D-023 listete sie
  ausdrücklich unter den erteilten Policy-Abilities („die `delete`/`restore`/
  `forceDelete` Policy-Abilities"). Sie wird **niemandem** mehr erteilt: endgültiges
  Entfernen macht ausschließlich der Prune-Job.

**Warum das besser ist:** Eine manuelle „endgültig löschen"-Schaltfläche wäre die
einzige Stelle im System, an der ein einzelner Klick Daten unwiederbringlich
vernichtet. Die gibt es jetzt nicht mehr — der Weg führt immer über die 30 Tage.

**Scheinwiderspruch, der keiner ist:** Bei D-138 wurde „Papierkorb / endgültiges
Löschen" als GF-exklusiv markiert, hier darf Management löschen. Das passt zusammen:
*endgültiges* Löschen darf **niemand**, auch die GF nicht. Löschen und
Wiederherstellen im Sinne des Papierkorbs sind eine andere Ebene.

**Agent-Entscheidung — was der Job nicht anfassen darf:** Rechtlich
aufbewahrungspflichtige Belege sind vom automatischen Entfernen **ausgenommen**.
Betrifft mindestens `Invoice` samt Positionen (GoBD-Unveränderlichkeit, D-093 —
eine gestellte Rechnung ist unveränderlich, erst recht unlöschbar) und alles daran
Hängende. Ein Job, der das nicht respektiert, wäre ein Rechtsverstoß per Cron.

**Offen:** die konkrete **Ausnahmeliste** für den Prune-Job — welche Entitäten neben
`Invoice` aufbewahrungspflichtig sind. Bei Umsetzung festzulegen, mit dem
Steuerberater abzustimmen (analog `BILLING.md` #2).

### D-140 — Preishoheit: Geschäftsführung und Management

**Status:** entschieden · **Datum:** 2026-09-13

Schreibrechte auf **`service_prices`** (Wartungspauschale, Stundensatz — D-062/D-065),
**`travel_zones`** (Fahrtzonenpauschale, D-063/D-084) und **`Article.sale_price` /
`Offering.sale_price`** (D-104) haben **nur** `geschaeftsfuehrung` und `management`.

**Nicht** `backoffice` — obwohl es den Artikelstamm pflegt (D-125). Backoffice legt
Artikel an und pflegt sie, **bepreist** sie aber nicht. **Nicht** `sales`.

**Abgrenzung, die oft verwechselt wird:** `discount_percent` an der einzelnen
Angebotsposition (D-052) ist **kein** Preisstamm, sondern Teil des Angebots — das
darf `sales` im Rahmen seiner `sales.*`-Rechte. Die Preishoheit betrifft die
**Listenpreise**, nicht den ausgehandelten Einzelfall.

---

## Bereich: Documents (Runde 2026-09-13)

Letzter ungegrilter Bereich. Ergebnis:
[`../04-domain/DOCUMENTS.md`](../04-domain/DOCUMENTS.md). Mündet aus `SALES.md` #6,
`BILLING.md` (Rechnung/Mahnung/e-Rechnung), `SERVICE.md` (Berichte, Fotos),
Inventory (Abhol-, Umbuchungs-, Rückgabebeleg, Zählauftrag) und ADR-029.

### D-141 — Belegvorlagen: Blade jetzt, AnkaReports als spätere Ausbaustufe

**Status:** entschieden · **Datum:** 2026-09-13 · löst die vertagte Designer-Notiz aus `DOMAIN.md`

Jede Belegart ist ein **Blade-Template**, gerendert nach HTML und von dort nach PDF.
Layoutänderung = Deployment. Für den MVP (ADR-032) der schnellste Weg, und bei rund
einem Dutzend Belegarten mit seltenen Layoutänderungen angemessen.

**Der visuelle Designer bleibt vorgemerkt — und zwar AnkaReports, nicht NextReports.**
[NextReports](https://github.com/nextreport/engine) ist eine **Java**-Engine und
bräuchte eine JVM neben PHP auf demselben Host: zweiter Technologie-Stack, eigener
Speicherbedarf, eigene Betriebssorge. [AnkaReports](https://github.com/ankareport/ankareport)
ist JavaScript und einbettbar.

**Konsequenz für den Bau:** Die Pipeline wird so geschnitten, dass die **Quelle des
Templates austauschbar** ist:

```text
eingefrorene/aktuelle Daten  ──┐
                               ├──> Renderer ──> HTML ──> PDF
Template ──────────────────────┘
   jetzt:  Blade-Datei
...
Der Renderer und die Datenbeschaffung bleiben gleich; nur woher das Template kommt,
ändert sich. Ein späterer Wechsel ist damit additiv, kein Umbau.

### D-142 — Keine archivierten PDFs, kein Template-Pin: JIT aus eingefrorenen Daten

**Status:** entschieden · **Datum:** 2026-09-13 · **bestätigt ADR-006**
...
**Die Frage war berechtigt und wurde bewusst so entschieden.** Aufgeworfen hatte sie
der Agent: ändert sich das Template, sieht eine neu gerenderte Rechnung anders aus als
die, die der Kunde erhalten hat. Nutzer: „Wir haben aktuell weder noch, und ich frage
mich auch, ob das ein Problem darstellen könnte, da wir eine sehr etablierte
Buchhaltung/ReWe haben, die das auch nicht macht."

**Einschätzung, die die Entscheidung trägt:**
...
- Die **GoBD verlangt Unveränderbarkeit der gebuchten Daten**, kein eingefrorenes
  ...
  Schriftart. Die Daten sind nach **D-093** eingefroren (Summen, Empfänger-Snapshot,
  Unveränderlichkeit ab `gestellt`), der Inhalt ist also reproduzierbar.
- **Bei der e-Rechnung ist die Datenstruktur ohnehin der Beleg.** XRechnung ist XML
  aus genau denselben Daten — dort ist „die Daten sind die Rechnung" keine Auslegung.
- Was in einer Betriebsprüfung Probleme macht, sind abweichende Beträge oder fehlende
  Pflichtangaben, nicht ein verschobenes Logo.
- Die Praxis einer etablierten Buchhaltung, die es genauso hält, ist ein belastbares
  Indiz.

**Bewusst dokumentiert, falls es später anders bewertet wird:** Sollte der
Steuerberater darauf bestehen, ist die Nachrüstung klein — entweder eine
...
eigenen Ausnahmeklausel bereits vor („sofern kein rechtlicher Grund für die Datei als
Primärrepräsentation besteht").

### D-143 — Ein „Dokument" ist ein Renderziel, keine Tabelle; Fotos und Uploads sind `Attachment`

**Status:** entschieden · **Datum:** 2026-09-13 · **revidiert `SERVICE.md`**

Nutzer: Documents sind **ausschließlich die erzeugten Belege**; Fotos und eingehende
Dateien werden getrennt modelliert.

**Die Konsequenz daraus, die leicht übersehen wird:** Zusammen mit D-142 („erzeugte
Belege werden nicht gespeichert") heißt das — **es gibt keine `documents`-Tabelle.**
Ein Dokument ist ein **Renderziel**, kein Datensatz. Der Bereich Documents besteht aus:

1. einem **Belegarten-Katalog** (D-144) — im Code, nicht in der Datenbank,
2. einem **Renderer** (Daten + Template → HTML → PDF),
3. **keinem** persistenten Dokumentmodell.

**Fotos und Uploads bekommen ein eigenes Modell (Agent-Entscheidung): `Attachment`**,
polymorph an den Vorgang gehängt, Datei auf dem gemounteten Volume (ADR-029). Das
ersetzt die bisherige Formulierung in `SERVICE.md` — „Fotos/Nachweise → `documents`,
polymorph am Report" ist damit **falsch** und wird korrigiert: Fotos sind keine
Dokumente.

`Attachment` trägt zunächst nur Einsatzfotos. Kundenseitige Uploads (Portal,
`ContactRequest`) können später dasselbe Modell nutzen — dann aber mit eigener
Zugriffsprüfung, weil sie nicht öffentlich sind (ADR-028/ADR-029).

### D-144 — Belegarten-Katalog: 13 Belege

**Status:** entschieden · **Datum:** 2026-09-13

Aus den bereits spezifizierten Bereichen ergeben sich diese Belegarten. Jede ist ein
Blade-Template (D-141) mit einer definierten Datenquelle:
...
Nicht enthalten: der **Zählauftrag** (D-113) — der ist eine Arbeitsliste in der
Oberfläche, kein Beleg zum Versenden. Wird er später doch gedruckt, kommt er als
eigene D-NNN dazu.

**Briefanrede** wird bei der Erzeugung **generiert**, nicht gespeichert (D-015) — der
Katalog hat keine eigene Anredelogik.

### D-145 — Aufräumregel für Einsatzfotos: weiter vertagt

**Status:** offen (vertagt) · **Datum:** 2026-09-13 · betrifft ADR-029

Nutzer: „Später entscheiden." Es wird **vorerst nichts automatisch gelöscht**; die
Regel kommt, wenn absehbar ist, wie schnell das Volume tatsächlich wächst.

**Bis dahin gilt:** das Volume wächst ungebremst. Der Plattenplatz auf dem gemeinsamen
Host ist zu beobachten (ADR-029), und die fehlende Sicherung des Volumes bleibt
ebenfalls offen.

---

### D-146 — Octane vertagt: Dev fährt FrankenPHP im klassischen Modus

**Frage:** ADR-022 legt Octane/FrankenPHP fest. Der aufgebaute Dev-Stack fährt
FrankenPHP jedoch im klassischen Request-pro-Prozess-Modus, nicht im Worker-Modus.
Octane jetzt nachziehen oder die Abweichung festschreiben?

**Entscheidung:** Octane wird **vertagt**. Der Nutzer will die Frage eigenständig
evaluieren („octane kommt später – das muss ich nochmal evaluieren"). Bis dahin
läuft FrankenPHP klassisch.

**Warum das im Dev ohnehin die richtige Wahl ist:** der Worker-Modus hält die
Anwendung zwischen Requests im Speicher. Genau das ist im Dev unerwünscht — der
ERP-Quellcode hängt per Bind-Mount im Container, und eine Codeänderung würde erst
nach einem Worker-Neustart wirken. Klassisch greift jede Änderung sofort.

**Status von ADR-022:** unverändert gültig als Zielbild für Produktion. Diese
Entscheidung ersetzt sie **nicht**, sie hält fest, dass der Dev-Stack sie heute noch
nicht umsetzt. Ob Octane in Produktion kommt und ob Dev nachzieht, ist offen und wird
bei der Evaluierung entschieden.

**Offen:** Evaluierung Octane (Nutzer). Dabei mitzuklären: falls Octane kommt, ob
Dev klassisch bleibt (dann steht in ADR-022 künftig „Dev klassisch, Prod Worker") oder
ob der Bind-Mount-Komfort im Dev anders gelöst wird.
